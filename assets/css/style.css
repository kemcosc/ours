...
/* ============================================
   CATEGORY BAR — second nav under main nav
   ============================================ */
.catbar {
  position: fixed;
  top: var(--nav-h);
...
.catbar a:hover { color: #fff; background: rgba(42,125,225,0.12); }
.catbar a.active { color: #fff; background: var(--accent); }
.catbar .sep { color: var(--text-dim); opacity: 0.35; font-size: 0.7rem; }

/* Push everything else down by the catbar height */
nav { /* unchanged */ }
.hero, .page-hero { padding-top: calc(var(--nav-h) + 44px + 60px); }
.page-hero { padding-bottom: 80px; }
.hero { padding-bottom: 80px; }

@media (max-width: 860px) {
  .catbar { height: 42px; }
  .catbar .container { justify-content: flex-start; padding: 0 16px; overflow-x: auto; }
  .catbar a { padding: 6px 12px; font-size: 0.76rem; letter-spacing: 0.08em; }
  .catbar .sep { display: none; }
}
